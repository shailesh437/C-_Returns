...
//Another   approach ....
int maxProfitTwixeBuySell(int price[],int n){
	int first_buy=INT_MIN;
	int second_buy=INT_MIN;
	int first_sell=0;
	int second_sell=0;
	for(int i=0;i<n;i++){ //Negation used for simple Math
		first_buy=max(first_buy,-price[i]);
		first_sell=max(first_sell,price[i]+first_buy);
		second_buy = max(second_buy,-price[i]+first_sell);
...
clock_t begin = clock();
	...
	while(TC--){
		...
		cout<<endl;
		cout<< maxProfitTwixeBuySell(price,7);
	}
	 #ifndef ONLINE_JUDGE 
      clock_t end = clock();
